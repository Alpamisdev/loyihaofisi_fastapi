...
def run_migration():
    """Update the news tables schema to ensure it supports multilingual content."""
    try:
        ...
        # Check if news_posts table exists
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='news_posts'")
        if not cursor.fetchone():
            logger.info("Creating news_posts table...")
            cursor.execute("""
        ...
        # Check if news_translations table exists
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='news_translations'")
        if not cursor.fetchone():
            logger.info("Creating news_translations table...")
            cursor.execute("""
        ...
        # Check if old news table exists and migrate data if needed
